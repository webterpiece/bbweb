	...
	class YITH_WC_Catalog_Mode_Premium extends YITH_WC_Catalog_Mode {
		...
		public function __construct() {
			...
			if ( get_option( 'ywctm_enable_plugin' ) == 'yes' && $this->check_user_admin_enable() ) {
				...
				if ( ! is_admin() || $this->is_quick_view() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
					...
					add_filter( 'woocommerce_get_price', array( $this, 'show_product_price' ), 10, 2 );
		...
		public function get_custom_button_template() {
			...
			if ( apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post->ID, '_ywctm_custom_url_enabled', true ), $post->ID, '_ywctm_custom_url_enabled' ) == 'yes' ) {
				...
				$target   = apply_filters( 'ywctm_get_vendor_postmeta', get_post_meta( $post->ID, '_ywctm_custom_url_link_target', true ), $post->ID, '_ywctm_custom_url_link_target' );
		...
		public function show_product_price( $price, $product ) {
			...
			if ( $hide_price == 'yes' && $this->apply_catalog_mode( $product->id ) ) {
				...
				if ( current_filter() == 'woocommerce_get_price' && ! $price ) {

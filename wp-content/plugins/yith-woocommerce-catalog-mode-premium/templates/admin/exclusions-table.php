					...
					switch ( $current_section ) {
						...
						case 'categories':
							...
							foreach ( $category_ids as $category_id ) {
								...
								YITH_WCTM()->update_term_meta( $category_id, '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), $add_to_cart );
								YITH_WCTM()->update_term_meta( $category_id, '_ywctm_exclude_hide_price' . $this->get_vendor_id(), $price );
						...
						case 'tags':
							...
							foreach ( $tag_ids as $tag_id ) {
								...
								YITH_WCTM()->update_term_meta( $tag_id, '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), $add_to_cart );
								YITH_WCTM()->update_term_meta( $tag_id, '_ywctm_exclude_hide_price' . $this->get_vendor_id(), $price );
			...
			if ( isset( $_GET['id'] ) && ! empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
				...
				switch ( $current_section ) {
					...
					case'categories':
						...
						$item          = array(
							'ID'          => $_GET['id'],
							'add_to_cart' => YITH_WCTM()->get_term_meta( $_GET['id'], '_ywctm_exclude_catalog_mode' . $this->get_vendor_id(), true ),
							'price'       => YITH_WCTM()->get_term_meta( $_GET['id'], '_ywctm_exclude_hide_price' . $this->get_vendor_id(), true ),
						);
